 ...
 * @LastEditTime: 2020-05-20 16:45:07
 */ 
//十一、继承与派生实验（4学时）
...
public:
    Teacher(string n,int id,string pro,string jt,string class_n,string class_nu ):name(n),id(id),profession(pro),job_title(jt),class_n(class_n),class_num(class_nu);
    ~Teacher();
    void ShowInfo()
    {
        cout << "name:" << name;
        ...
        cout << "class name:" << class_n;
        cout << "class number:" << class_num;
    }
};
Teacher::~Teacher()
{
...
public:
    TA(string n,int id,string pro):name(n),id(id),profession(pro);
    ~TA();
    void ShowInfo()
    {
        cout << "name:" << name;
